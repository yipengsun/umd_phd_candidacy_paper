...
The signal and normalization yields are extracted from an extended, unbinned maximum-likelihood fit to two-demisional $m^2_{miss}-|\vec{p}^*_\ell|$ distributions, with $D^{(*)}\ell$ and $D^{(*)}\pi^0\ell$ as inputs \cite{Lees:2013uzd}.
