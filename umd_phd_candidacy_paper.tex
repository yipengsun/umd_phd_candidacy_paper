...
$B$ factories has advantages, but were limited by size of data sample.
BELLE is undergoing an upgrade in the hope to solve the problem.
At the same time, more LHCb measurements are in the works, such as measuring
both \RD/ and \RDst/ with run 1 data with the same semileptonic $B$ decay modes.

% Expected improvements
% Remember: Run 2 supposedly has better pile-ups.
% Current progress
The run 2 data sample size is expected to be around 4 times larger than that of
the run 1, due to higher integrated luminosity and larger $B \bar{B}$ production
cross section.
In an early study of the LHCb run 2 data, shown in \autoref{fig:lhcb_run2}, the
data sample size is up to 5 times larger.
More work is needed to validate the claim.
